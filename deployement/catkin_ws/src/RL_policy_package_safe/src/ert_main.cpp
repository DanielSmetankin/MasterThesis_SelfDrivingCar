...
#include "RL_AGENT_SAFE_POLICY.h"      // Model header file
#include <ros/ros.h>
...
static RL_AGENT_SAFE_POLICY RL_AGENT_SAFE_POLICY_Obj;// Instance of model class
...
void rt_OneStep(void)
{
  ...
  // Check for overrun
  if (OverrunFlag) {
    rtmSetErrorStatus(RL_AGENT_SAFE_POLICY_Obj.getRTM(), "Overrun");
    return;
  }
...
int_T main(int_T argc,  char **argv)
{
  ...
  // Initialize model
  RL_AGENT_SAFE_POLICY_Obj.initialize();
  ...
  while (rtmGetErrorStatus(RL_AGENT_SAFE_POLICY_Obj.getRTM()) == (nullptr)&& !g_shutdown ) {
    ...
    RL_AGENT_SAFE_POLICY_Obj.step(g_relativeDistance,chatter_pub,velocity,elapsedTime);
    
    
  }

  // Terminate model
  RL_AGENT_SAFE_POLICY_Obj.terminate();
  ros::shutdown();
  return 0;
}
